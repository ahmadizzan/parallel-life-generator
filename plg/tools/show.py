...
async def _build_rich_tree_level(node: BranchNode, tree: Tree, session: Session):
    """
    ...
    # 1. Fetch annotations for the current node.
...
async def generate_tree_view(start_node_id: int, session: Session) -> Tree:
    ...
    await _build_rich_tree_level(start_node, tree, session)
    return tree
